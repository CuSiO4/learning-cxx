...
int main(int argc, char **argv) {
    {
        ...
        // sizeof 获取栈上地址，vec数据在堆上，栈上只有三个 8B 的指针
    ...
    }
    {
        ...
        {
            // THINK: `std::vector` 插入删除的时间复杂度是什么？n
            auto it_2 = vec.begin() + 1;
            vec.insert(it_2, 1.5);
            ASSERT((vec == std::vector<double>{1, 1.5, 2, 3, 4, 6}), "Make this assertion pass.");
            auto it_3 = vec.begin() + 3;
            vec.erase(it_3);
            ASSERT((vec == std::vector<double>{1, 1.5, 2, 4, 6}), "Make this assertion pass.");
        }
        {
            vec.shrink_to_fit(); //让 vector 的 capacity（容量）缩小到正好等于当前的 size（元素个数
